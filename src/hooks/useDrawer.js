...
export const useDrawer = () => {
    ...
    const hadlerOrder = async () => {
        try {
            await axios.post(`${API2}/orders`, {items: cart.data}).catch((e) => {throw new Error(e)});
        } catch (error) {
            return error;
        }
    }
    ...
    const handlerRemoveItems = async () => {
        try {
            await hadlerOrder().catch((e) => {throw new Error(e)});

            for (const item of cart.data) {
                await axios.delete(`https://6543a8f001b5e279de20c076.mockapi.io/cart/${item.id}`).catch((e) => {throw new Error(e)});
            }
    ...
    const removeItem = async (id, parentId) => {
        try {
            const response = await axios.get(`${API}cart`).catch((e) => {throw new Error(e)});
            const filterId = response.data.filter((item) => +item.id === +id);
    
            await axios.delete(`${API}cart/${filterId[0].id}`).catch((e) => {throw new Error(e)});
    ...
    return {status, setStatus, handlerDrawer, cart, setCart, handlerRemoveItems, removeItem, handlerAreaDrawer, orders}
}