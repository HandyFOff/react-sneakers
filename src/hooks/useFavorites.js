...
export const useFavorites = () => {
    ...
    const handlerAddToFavorite = async (id, title, price, image, parentId) => {
        try {
            const item = await axios.post(`${API2}/favorites`, { id, title, price, image, parentId }).catch((e) => {throw new Error(e)})
    ...
    const handlerRemoveFromFavorite = async (active, id) => {
        try {
            const response = await axios.get(`${API2}favorites`).catch((e) => {throw new Error(e)});
            ...
            await axios.delete(`${API2}favorites/${filterId[0].id}`).catch((e) => {throw new Error(e)});
    ...
    const handlerFavorite = async (id, title, price, image, parentId, active) => {
        if (isItemFavorited(parentId) || active) await handlerRemoveFromFavorite(active, id);
